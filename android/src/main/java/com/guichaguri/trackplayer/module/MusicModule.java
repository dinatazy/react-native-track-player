...
public class MusicModule extends ReactContextBaseJavaModule implements ServiceConnection {
    ...
    @ReactMethod
    public void getCenterFreq(final float band, final Promise callback) {
     waitForConnection(() -> {
        callback.resolve(binder.getPlayback().getCenterFreq(band));
        return;
    });
    }
    ...
    @ReactMethod
    public void getCurrentPreset(final Promise callback) {
     waitForConnection(() -> {
        callback.resolve((float) binder.getPlayback().getCurrentPreset());
        return;
    });
    }

    @ReactMethod
    public void getNumberOfBands(final Promise callback) {
     waitForConnection(() -> {
        callback.resolve((float) binder.getPlayback().getNumberOfBands());
        return;
    });
    }
    ...
    @ReactMethod
    public void setBandLevel(float band, float level, final Promise callback) {
     waitForConnection(() -> {
        binder.getPlayback().setBandLevel(band, level);
        callback.resolve(null);
    });
    }
}
